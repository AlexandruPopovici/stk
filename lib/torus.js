function Torus (largeRadius, smallRadius) {
  var vertices = [];
  var indices = [];
  var normals = [];
  var uvs = [];
  ...
  var largeRadius = largeRadius;
  var smallRadius = smallRadius;
  var minWSegments = 30;
  var minHSegments = 30;
  ...
  for (var s = 0; s < minWSegments; ++s, a1a = a1b, a1b += step1r) {
    ...
    for (var s2 = 0; s2 < minHSegments; ++s2, a2a = a2b, a2b += step2r) {
      ...
      var xd = step1r/(2*Math.PI);
      var yd = step2r/(2*Math.PI);
      uvs.push(s/minWSegments, s2/minHSegments);
      uvs.push(s/minWSegments+xd, s2/minHSegments);
      uvs.push(s/minWSegments+xd, s2/minHSegments+yd);
      uvs.push(s/minWSegments, s2/minHSegments+yd);
      vCount += 4;
    }
  }
