    ...
    function makePlane(u, v, w, su, sv, nu, nv, pw, flipu, flipv) {
        ...
        for(var j=0; j<=nv; j++) {
            for(var i=0; i<=nu; i++) {
                ...
                vert[w] = pw;
                positions[vertexIndex*3 + u] = vert[u];
                positions[vertexIndex*3 + v] = vert[v];
                positions[vertexIndex*3 + w] = vert[w];

                var normal = [0,0,0];
                normal[u] = 0
                normal[v] = 0
                normal[w] = pw/Math.abs(pw);
                normals[vertexIndex*3+u] = normal[u];
                normals[vertexIndex*3+v] = normal[v];
                normals[vertexIndex*3+w] = normal[w];

                var texCoord = [0,0];
                texCoord[0] = i/nu;
                texCoord[1] = 1.0 - j/nv;
                uvs[vertexIndex*2 + 0] = texCoord[0];
                uvs[vertexIndex*2 + 1] = texCoord[1];
